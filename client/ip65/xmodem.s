...
xmodem_receive:
  ...
  stax got_byte+1
  jsr xmodem_transfer_setup
  jsr send_nak   		; nack starts the download
...
:
  ...
  adc checksum			; Conditionalize Checksum Update... If CRC do CRC 
  sta checksum			; otherwise this simple one
  ...
  lda #XMODEM_TIMEOUT_SECONDS	; CRC received Checksum is 2 bytes so may need routine for this
  jsr getc
...
getc:
  jsr @real_getc
  sta $0600
  bcc :+  ;  Carry Set we got an error, then bail
  rts
:  
  cmp #$ff
  beq @got_ff
  clc   			; not FF so clear the carry and return
  rts
@got_ff:    
...
CRCLO   EQU >checksum_crc          ; current value of CRC
CRCHI   EQU <checksum_crc          ; not necessarily contiguous

	;;  Called once per byte of message, Hi and low contain result clobbers AXYP
...
got_eot: .byte "end of transmission ",07,07,07,10,0
