...
.include "../inc/common.i"
  ...
  .import outbound_data
  .import outbound_data_len
  .import buffer_toggle
  .import view_buffer
  .import print_buffer
  .import clear_buffer
  .import write_to_buf
...
.export echo_flag
.export scratch_buffer
.export display_echo_flag
...
;inputs:
;telnet_use_native_charset: set to 0 if remote server uses standard ASCII, 1 if remote server uses the 'native' charset (i.e. PETSCII) 2 if ANSI
...
telnet_connect:
  ...
  lda telnet_use_native_charset	; 0= vt100 1=petsci 2=ansi
  cmp #$01
  beq :++  			; petscii, set up petscii
  cmp #$00                      ; vt100
  bne :+
  sec
  jmp @init_vt100
:
  clc                           ; otherwise it was 2 for ansi
@init_vt100:
...
:
  ...
  jsr $ff62 			; reset char set to petscii/init
  lda #$00
  sta $f8	                ; turn on wrapping/scrolling
  sta $f7			; turn off character set locks
continue:	
...
@connect_ok:
  ...
  sta iac_response_buffer_length ; reset IAC buffer to empty
...
@disconnected:
  lda #$00
  sta $f8	                ; turn on wrapping/scrolling when the connection
	;;  closes
  ldax #disconnected
  jsr print
  rts
@not_disconnected:                 
  ...
  jsr tcp_send			; send the IAC responses
	
@no_iac_response:


	;;  if anything to be sent from processing inbound, send it

	
  lda outbound_data_len
  beq @keep_going  	;nothing to send just exit
  ldx #$00
  stax tcp_send_data_len
  stx outbound_data_len
  ldax  #outbound_data
  jsr tcp_send
...
@not_view_help:
  ...
  jsr echo_char             	; echo outbound char if echo set  
  ldx telnet_use_native_charset
  cpx #$01
  beq @no_conversion_required
...
@next_byte:
  ...
  lda (buffer_ptr),y
  tax
  lda telnet_use_native_charset	; made it a tri flag 0= vt100 1=petscii 2=ansi
  cmp #$01
  bne :+
...
echo_char:
  ...
  pha	
  lda #$C0
  sta $f8	                ; turn off wrapping/scrolling
  pla	
  jmp :++
:
  pla
...
;constants
xclosing_connection: .byte 5,"CLOSING CONNECTION",13,0
disconnected: .byte 13,5,"CONNECTION CLOSED",13,0
transmission_error: .byte 5,"ERROR WHILED SENDING ",0
