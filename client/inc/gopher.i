...
: lda dns_ip,x
  ...
  stax  telnet_port
  lda #0
  sta telnet_use_native_charset  ; vt100
  
  ;if the username = '/n', then connect in native mode
  lda resource_selector_length
  cmp #2                ; if its only 2 long assume its native no matter what
  bne @not_native
  ...
  jmp @go
@not_native:
  cmp #04  ; Ansi  ; assume if its 4 long its ANSI
  bne @vt100
  lda #$02
  sta telnet_use_native_charset
  jmp @go
